...
| **二期**（**已交付 2026-09-08**，D31） | ~~`SocketNotifier` 实现（uv_poll 底座）~~ → **按 F6 裁定以 `register/unregister_socket_notifier` 虚函数形态交付于 `AbstractEventDispatcher`**（独立 RAII 类 deferred——TcpSocket 直消费 dispatcher API 无独立消费者，YAGNI）；`TcpSocket/TcpServer` 已交付（memcached 状态机内核 + beast flat_buffer 形状读缓冲，TcpServer accept 走 `adopt_uv_tcp`）；UdpSocket/PipeStream 未做（需求触发）；timer 精化小件包部分交付：0ms 直投已落（T0） | 二期实际签名对齐：`register_socket_notifier(int fd, SocketEventMask mask, std::function<void(SocketEventMask)> fn)` / `unregister_socket_notifier(int fd)`——回调带 fired mask（curl event_bitmap 回灌对齐，F6 deviation ②），**非本行字面 `std::function<void()>`**；基类默认实现 = `CXXKIT_CHECK(false, E1 文案)`（F6 deviation ① fail-loud，非「默认空实现」）；std::function 而非 signals（F6 deviation ③）；uv/qt 双侧落位（qt 侧 CHECK(false) 不支持）；TcpSocket 构造显式传 EventLoop（asio 式无虚基类，三期模板化留门如初）；对账完成记录于 decisions.md D31 |
