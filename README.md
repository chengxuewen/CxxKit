...
| `cxxkit::network` | compiled | http (cpr backend); TcpSocket/TcpServer + TlsSocket (mbedTLS over the transport, backend-agnostic) event-loop sockets with Qt-grade error/state surface — backend selection `CXXKIT_NETWORK_BACKEND` (uv default, asio opt-in) |
